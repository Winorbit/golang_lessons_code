...
func main() {
	...
	myWord := "smoke"
	for _, sentence := range quotes{
		if strings.Contains(sentence.Text, myWord){
			fmt.Println(sentence.Text)}};


	// IF
	// Вынесем условие в отдельную переменную
	myWord := "smoke"
	for _, sentence := range quotes{
		wordInSentence := strings.Contains(sentence.Text, myWord)
		if wordInSentence{
			fmt.Println(sentence.Text)}};
	...
	for _, sentence := range quotes{
		wordInSentence := strings.Contains(sentence.Text, myWord)
		switch wordInSentence{
		  case true: fmt.Println(sentence.Text)}};
