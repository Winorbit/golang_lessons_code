...
func addUserToSession(user User){
  _, sessionExist := sessions[user.Id]
  if !sessionExist{
    newUser := User{Id: user.Id, Username: user.Username}
    sessions[user.Id] = &newUser
  }
}
...
func processMessages(user_id int, chatId string, message string, token string){
  ...
  if Contains(botCommands, message){
    ...
    if message == "/my_level"{
      currentLevel := strconv.Itoa(userFromSession.Level)
      levelMessage := fmt.Sprintf("Your current english lecel is %v", currentLevel)
      sendMessage(chatId, levelMessage, token)
    }
  }
